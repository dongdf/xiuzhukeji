...
Page({
      ...
      wx.showActionSheet({
        ...
        success(res) {
          if (res.tapIndex == 0) {
            wx.showModal({
              title: '确定取消会议吗？',
              content: '取消约会，会议保证金会在三个工作日内进行原路返回',
              success: function (res) {
                if (res.confirm) {
  ...
  getmeber() {
    getApp().get('member/lists?id=' + this.data.meetid + 'page=1&size=8').then(res => {
      if (res) {
        this.setData({
          memberlist: res
        })
  ...
  gethbList: function () {
    ...
    getApp().get('member/hongbao?page=1&size=1000&late=1&id=' + this.data.meetid).then(res => {
      var t = 0;
      var all = res.get_red_pack.length + res.not_get_red_pack.length;
      if(all>0){
        var argtemp = res.late_money / all;
        this.setData({
          avg: argtemp
        })
      }else{
